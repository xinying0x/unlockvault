...
const GamesPage: React.FC = () => {
  ...
  const fetchSettingsAndOffers = async () => {
    try {
      ...
      // تصفية الألعاب فقط (وليس الأدوات أو التطبيقات)
      const gamesOnly = offersData.filter((item: Offer) => item.type === 'game');
      setOffers(gamesOnly);
      setLoading(false);
      ...
      addToast({
        type: 'success',
        title: 'Games Loaded',
        message: `Found ${gamesOnly.length} premium games`
      });
    } catch (error) {
      console.error('فشل جلب الألعاب أو الإعدادات:', error);
      setLoading(false);
      addToast({
        type: 'error',
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#18122B] via-[#2D1B5A] to-[#1A1A2E] text-white">
        <SEOHead
          title="Premium Games - UnlockVault"
          description="Browse our collection of premium games and hacks"
          keywords={['premium games', 'game hacks', 'cracked games']}
          url="https://unlockvault.com/games"
        />
        
        <div className="container mx-auto px-4 py-20">
          <div className="text-center mb-12">
            <div className="w-16 h-16 border-4 border-purple-500 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
            <h2 className="text-2xl font-bold text-white mb-2">Loading Premium Games</h2>
            <p className="text-gray-400">Please wait while we fetch the latest games...</p>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#18122B] via-[#2D1B5A] to-[#1A1A2E] text-white">
      <SEOHead
        title="Premium Games - UnlockVault"
        description="Browse our collection of premium games and hacks for free. Find FPS, MOBA, Battle Royale game hacks and more."
        keywords={['premium games', 'game hacks', 'cracked games', 'free games', 'FPS hacks', 'MOBA hacks']}
        url="https://unlockvault.com/games"
      />
      ...
      <section className="py-20 px-4 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          <h1 className="text-5xl md:text-6xl font-extrabold mb-6 bg-gradient-to-r from-red-400 via-pink-400 to-rose-400 bg-clip-text text-transparent animate-fade-in-up">
            Premium Games & Hacks
          </h1>
          <p className="text-xl md:text-2xl text-gray-300 mb-8 max-w-3xl mx-auto leading-relaxed animate-fade-in-up delay-200">
            Dive into our vast collection of premium game hacks, cheats, and cracked games, all available for free. Enhance your gaming experience and dominate the competition.
          </p>
        </div>
      </section>
