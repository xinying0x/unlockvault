        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16 sm:h-20">
            ...
            <div className="hidden lg:flex items-center space-x-1">
              {navLinks.filter(l => l.href !== '/scan-qr').map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
              ...
              {/* Scan QR — highlighted CTA */}
              <Link
                href="/scan-qr"
                className="relative ml-3 flex items-center gap-2 px-5 py-2 bg-purple-600 hover:bg-purple-500 text-white text-sm font-semibold rounded-xl transition-all duration-200 hover:scale-105 focus:outline-none"
              >
                Scan QR
        ...
        <div className={`lg:hidden transition-all duration-300 ease-in-out ${
          ...
          <div className="bg-slate-900/98 backdrop-blur-xl border-t border-slate-700/50">
            <div className="px-4 py-6 space-y-2">
              {navLinks.filter(l => l.href !== '/scan-qr').map((link, index) => (
                <Link
                  key={link.href}
                  href={link.href}
                  onClick={closeMenu}
                  className={`block px-4 py-3 rounded-xl font-medium transition-all duration-200 focus:outline-none ${
                    isActiveLink(link.href)
                      ? 'text-white bg-white/10'
                      : 'text-gray-400 hover:text-white hover:bg-white/5'
                  }`}
              ...
              <Link
                href="/scan-qr"
                onClick={closeMenu}
                className="block px-4 py-3 rounded-xl font-semibold bg-purple-600 hover:bg-purple-500 text-white transition-all duration-200 focus:outline-none"
              >
